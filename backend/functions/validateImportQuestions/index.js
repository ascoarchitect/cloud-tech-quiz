    ...
    try {
      ...
      questions.forEach((question, index) => {
        ...
        if (!question.options || question.options.length < 2) {
          ...
          question.options.forEach((option) => {
            if (optionIds.has(option.id)) {
              errors.push(`Question #${questionIndex}: Duplicate option ID - ${option.id}.`);
            }
            optionIds.add(option.id);
            
            // Check if option text is provided
            if (!option.text) {
              errors.push(`Question #${questionIndex}, Option ${option.id}: Missing option text.`);
            }
          });
        }
        ...
        if (!question.correctAnswer) {
          ...
          errors.push(`Question #${questionIndex}: Correct answer does not match any option ID.`);
        }
