...
class AntiCheatModule {
  ...
  public start(): void {
    ...
    this.addEventHandler(document, 'contextmenu', this.preventContextMenu.bind(this));
    
    // Detect print attempts
    this.addEventHandler(window, 'beforeprint', this.handlePrintAttempt.bind(this));
  ...
  private handleFocusChange(event: Event): void {
    ...
    if (event.type === 'blur') {
      ...
      if (this.focusChangeCount > 3) {
        this.triggerWarning('Multiple tab switches detected. Please stay in the test window.');
      }
      ...
      if (timeAway > 10000 && this.lastFocusTime > 0) {
        this.triggerWarning(`Test window was inactive for ${Math.round(timeAway / 1000)} seconds.`);
      }
    }
  }
  ...
  private preventCopyPaste(event: Event): void {
    ...
    this.triggerWarning('Copy/paste operations are not allowed during the test.');
  }
